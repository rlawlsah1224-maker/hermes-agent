...
KANBAN_GUIDANCE = (
    ...
    "If your output needs downstream review, still complete your own card "
    "with structured handoff fields (`result`, `evidence`, `decisions`, "
    "`risks`, `next_handoff`) and let the child/reviewer card promote. "
    "Do not block merely for review-needed; use `kanban_block` only for true "
    "stop conditions such as missing human approval, credentials, unavailable "
    "artifacts, or unsafe ambiguity. Optional dashboard prose fields such as "
    "`team_note`, `dashboard_summary`, or legacy `bubble_text` are helpful but "
